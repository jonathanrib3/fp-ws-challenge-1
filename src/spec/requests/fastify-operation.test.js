...
import { buildFastifyApp } from "../../buildFastifyApp.js";

const app = buildFastifyApp();

describe("/:operation/ping request tests", (t) => {
  describe("when checking server status passing an operation as params", () => {
    it("returns a 200 http status response and a body containing the requested operation", async () => {
      const response = await app.inject({
        method: 'GET',
        url: '/sum/ping'
      });
      const expectedBody = {
        message: 'So you want to perform a sum operation huh?'
      };
